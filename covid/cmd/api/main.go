...
func main() {
	...
	conn, err := grpc.Dial("covid-service:3000", grpc.WithInsecure())
	if err != nil {
		panic(err)
	}
	defer conn.Close()

	covidClient := v1.NewCovidServiceClient(conn)
	...
	routes.HandleFunc("/add/{a}/{b}", func(w http.ResponseWriter, r *http.Request) {
		...
		req := &v1.AddRequest{Api: "v1", Covid: &v1.Covid{A: a, B: b}}

		if resp, err := covidClient.Add(ctx, req); err == nil {
